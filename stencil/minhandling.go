...
func (s Stencil) HandleMin(i image.Image) image.Image {
	...
	case Repeat:
		return s.repeat(i)
	case StretchAspect:
		return s.stretchAspect(i)
	}
}

func (s Stencil) OutMin(bounds image.Rectangle) bool {
	return s.MinHeight != nil && *s.MinHeight > bounds.Max.Y || s.MinWidth != nil && *s.MinWidth > bounds.Max.X
}

func (s Stencil) stretch(i image.Image) image.Image {
	bounds := i.Bounds()
...
func (s Stencil) repeat(i image.Image) image.Image {
	bounds := i.Bounds()
	var endX, endY = bounds.Dx(), bounds.Dy()
	if s.MinHeight != nil && *s.MinHeight > endY {
		endY = *s.MinHeight
...
func (s Stencil) stretchAspect(i image.Image) image.Image {
	bounds := i.Bounds()
	var endX, endY = bounds.Dx(), bounds.Dy()
